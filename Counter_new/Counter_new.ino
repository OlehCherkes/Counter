...
#define TFT_CS 9 // цифровой выход D15
#define TFT_DC 4 // цифровой выход D2
...
void setup() {
  ...
  EEPROM.get(0, coils_turns);
  EEPROM.get(4, my_data.diameter);
  EEPROM.get(8, my_data.width_);
  EEPROM.get(12, my_data.wire);
  EEPROM.get(16, my_data.rotary);
...
void angle() {
  ...
  if (flag_angle == false)
  {
    ...
    if (encoder >= 0) {
      tft.print(constrain(encoder / 2, 0, 360));
    }
    else tft.print(constrain(encoder / 3, -360, 0));
...
void printSetings(int pointer_value) {
  ...
  EEPROM.put(4, my_data.diameter);
  EEPROM.put(8, my_data.width_);
  EEPROM.put(12, my_data.wire);
  EEPROM.put(16, my_data.rotary);
...
// фунция заряда батареи
void battery_charge() {
  static int prev_step_battary;
  tft.drawBitmap(425, 10, Battery, 25, 25, 0xFFFF);
  if (analogRead(BATTERY) > 800) step_battary = 1;
  else if (analogRead(BATTERY) > 780 && analogRead(BATTERY) <= 800) step_battary = 2;
  else if (analogRead(BATTERY) > 740 && analogRead(BATTERY) <= 780) step_battary = 3;
  else if (analogRead(BATTERY) <= 740) step_battary = 4;
...
void encoderPinChangeA() {
  ...
  {
    encoder += (PIND & (1 << encoder_a)) == (PIND & (1 << encoder_b)) ? -1 : 1;
  }
  else
  {
    encoder += (PIND & (1 << encoder_a)) != (PIND & (1 << encoder_b)) ? -1 : 1;
